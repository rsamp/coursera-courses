...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toSeq.groupBy(identity).view.mapValues(_.size).toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.toMap.foldLeft(x.toMap)(
      (acc, occurrence) => acc.updated(occurrence._1, acc(occurrence._1) - occurrence._2)
    ).filter(_._2 != 0).toList.sorted
  }
  ...
  def sentenceAnagramsHelper(occurrences: Occurrences): List[Sentence] = {
    if (occurrences.isEmpty) List(List())
      ...
      for {
        ...
        rest <- sentenceAnagramsHelper(subtract(occurrences, combo))
      } yield word :: rest
    }
  }
}
