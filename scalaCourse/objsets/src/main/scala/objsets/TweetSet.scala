...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty) elem
      ...
      if (left.isEmpty) {
        ...
        mostRetweets(elem, rightMost)
      } else if (right.isEmpty) {
        val leftMost = left.mostRetweeted
        mostRetweets(elem, leftMost)
      } else {
        val leftMost = left.mostRetweeted
        val rightMost = right.mostRetweeted
        mostRetweets(mostRetweets(elem, leftMost), rightMost)
      }
    }
  }

  def mostRetweets(t1: Tweet, t2: Tweet): Tweet = {
  ...
  def descendingAcc(tweetSet: TweetSet, acc: TweetList): TweetList = {
    if (tweetSet.isEmpty) acc
    else {
      val most = tweetSet.mostRetweeted
      val rest = tweetSet.remove(most)
      descendingAcc(rest, acc.add(most))
    }
  }
...
object Nil extends TweetList {
  ...
  def isEmpty = true
  def add(tweet: Tweet) = new Cons(tweet, Nil)
}

class Cons(val head: Tweet, val tail: TweetList) extends TweetList {
  def isEmpty = false
  def add(tweet: Tweet) = new Cons(head, tail add tweet)
}
...
object GoogleVsApple {
   ...
   */
  lazy val trendingSet: TweetSet = googleTweets.union(appleTweets)
  lazy val trending: TweetList = trendingSet.descendingByRetweet
}

object Main extends App {
  // Print the trending tweets
//  GoogleVsApple.trendingSet foreach println
